...
#ifdef WIN32
  BLI_path_join(
      path, path_maxncpy, caches_root_path, "Bluffingo", "Bluffingos Fork", "Cache", SEP_STR);
#elif defined(__APPLE__)
  BLI_path_join(path, path_maxncpy, caches_root_path, "Bluffingos Fork", SEP_STR);
#else /* __linux__ */
  BLI_path_join(path, path_maxncpy, caches_root_path, "Bluffingos Fork", SEP_STR);
#endif
...
bool BKE_appdir_app_is_portable_install(void)
{
  ...
  char path[FILE_MAX];
  return get_path_local(path, sizeof(path), "config", nullptr);
}
    ...
    if (GetModuleFileNameW(0, fullname_16, program_filepath_maxncpy)) {
      ...
      if (!BLI_exists(program_filepath)) {
        CLOG_ERROR(&LOG,
                   ...
                   program_filepath);
        MessageBox(
            nullptr, "path contains invalid characters or is too long (see console)", "Error", MB_OK);
      }
