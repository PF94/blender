    ...
    tSplineIK_Tree *tree, Object *ob, bPoseChannel *pchan, int index, tSplineIk_EvalState *state)
{
  ...
  if (pchan->bone->length < FLT_EPSILON) {
    ...
    BKE_where_on_path(ik_data->tar, state->curve_position, bone_pos, nullptr, nullptr, &rad, nullptr);
...
void BKE_pose_pchan_index_rebuild(bPose *pose)
{
  ...
  for (bPoseChannel *pchan =
           static_cast<bPoseChannel *>(pose->chanbase.first); pchan != nullptr; pchan = pchan->next) {
...
void BKE_pose_eval_init(struct Depsgraph *depsgraph, Scene* /*UNUSED(scene)*/, Object *object)
{
  ...
  for (bPoseChannel *pchan = static_cast<bPoseChannel *>(pose->chanbase.first);
       pchan != nullptr; pchan = pchan->next) {
