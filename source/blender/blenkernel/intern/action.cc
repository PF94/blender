...
static void action_copy_data(Main /* UNUSED(bmain) */, ID *id_dst, const ID *id_src, const int flag)
{
  ...
  for (fcurve_src = static_cast<FCurve *>(action_src->curves.first); fcurve_src; fcurve_src = fcurve_src->next) {
    /* Duplicate F-Curve. */
...
IDTypeInfo IDType_ID_AC = {
    ...
    .init_data = nullptr,
    .copy_data = reinterpret_cast<IDTypeCopyDataFunction>(action_copy_data),
...
bPoseChannel *BKE_pose_channel_find_name(const bPose *pose, const char *name)
{
  ...
  return static_cast<bPoseChannel*>(BLI_findstring(&pose->chanbase, name, offsetof(bPoseChannel, name)));
}
...
bool BKE_action_has_single_frame(const struct bAction *act)
{
  ...
  LISTBASE_FOREACH (FCurve *, fcu, &act->curves) {
    ...
    const float this_key_frame = fcu->bezt != nullptr ? fcu->bezt[0].vec[1][0] : fcu->fpt[0].vec[0];
    if (!found_key) {
      found_key = true;
...
bool BKE_pose_copy_result(bPose *to, bPose *from)
{
  ...
  for (pchanfrom = static_cast<bPoseChannel *>(from->chanbase.first); pchanfrom; pchanfrom = pchanfrom->next) {
