...
static void armature_copy_data(Main */*UNUSED(bmain)*/, ID *id_dst, const ID *id_src, const int flag)
{
  ...
  for (bone_src = static_cast<Bone*>(armature_src->bonebase.first); bone_src; bone_src = bone_src->next) {
  ...
  for (bone_dst = static_cast<Bone*>(armature_dst->bonebase.first); bone_dst; bone_dst = bone_dst->next) {
...
void BKE_armature_copy_bone_transforms(bArmature *armature_dst, const bArmature *armature_src)
{
  ...
  const Bone *bone_src = static_cast<Bone *>(armature_src->bonebase.first);
...
static void allocate_bbone_cache(bPoseChannel *pchan, int segments)
{
  ...
  if (runtime->bbone_segments != segments) {
    ...
    runtime->bbone_rest_mats = static_cast<struct Mat4 *>(MEM_malloc_arrayN(
        1 + (uint)segments, sizeof(Mat4), "bPoseChannel_Runtime::bbone_rest_mats"));
    runtime->bbone_pose_mats = static_cast<struct Mat4 *>(MEM_malloc_arrayN(
        1 + (uint)segments, sizeof(Mat4), "bPoseChannel_Runtime::bbone_pose_mats"));
    runtime->bbone_deform_mats = static_cast<struct Mat4 *>(MEM_malloc_arrayN(
        2 + (uint)segments, sizeof(Mat4), "bPoseChannel_Runtime::bbone_deform_mats"));
    runtime->bbone_dual_quats = static_cast<struct DualQuat *>(MEM_malloc_arrayN(
...
void BKE_pose_rebuild(Main *bmain, Object *ob, bArmature *arm, const bool do_id_user)
{
  ...
  if (ob->pose == nullptr) {
    /* create new pose */
    ob->pose = static_cast<struct bPose *>(MEM_callocN(sizeof(bPose), "new pose"));
...
static void boundbox_armature(Object *ob)
{
  ...
  if (ob->runtime.bb == nullptr) {
    ob->runtime.bb = static_cast<struct BoundBox *>(MEM_callocN(sizeof(BoundBox), "Armature boundbox"));
  }
  bb = ob->runtime.bb;
...
bool BKE_pose_minmax(Object *ob, float r_min[3], float r_max[3], bool use_hidden, bool use_select)
{
  ...
  if (ob->pose) {
    ...
    for (pchan = static_cast<bPoseChannel *>(ob->pose->chanbase.first); pchan; pchan = pchan->next) {
      /* XXX pchan->bone may be nullptr for duplicated bones, see duplicateEditBoneObjects() comment
