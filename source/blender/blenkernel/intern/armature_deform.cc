                                          ...
                                          const MDeformVert *dvert)
{
  ...
  if (use_dverts && dvert && dvert->totweight) { /* use weight groups ? */
    ...
    if (deformed == 0 && use_envelope) {
      for (pchan = static_cast<const bPoseChannel *>(data->ob_arm->pose->chanbase.first); pchan; pchan = pchan->next) {
                                        ...
                                        const TaskParallelTLS *__restrict /* UNUSED(tls) */)
{
  ...
  const MDeformVert *dvert = static_cast<const MDeformVert *>(BM_ELEM_CD_GET_VOID_P(v, data->bmesh.cd_dvert_offset));
